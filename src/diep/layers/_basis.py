...
class SphericalBesselFunction(nn.Module):
    ...
    def rbf_j0(r, cutoff: float = 5.0, max_n: int = 3):
        ...
        """
        n = (torch.arange(1, max_n + 1)).type(dtype=diep.float_th)[None, :]
        r = r[:, None]
    ...
    def __init__(self, max_n: int, cutoff: float, learnable: bool = False):
        ...
        if learnable:
            self.frequencies = torch.nn.Parameter(
                data=torch.Tensor(pi * torch.arange(1, self.max_n + 1, dtype=diep.float_th)),
                requires_grad=True,
            )
        else:
            self.register_buffer(
                "frequencies",
                pi * torch.arange(1, self.max_n + 1, dtype=diep.float_th),
            )
...
class FourierExpansion(nn.Module):
    ...
    def __init__(self, max_f: int = 5, interval: float = pi, scale_factor: float = 1.0, learnable: bool = False):
        ...
        if learnable:
            self.frequencies = torch.nn.Parameter(
                data=torch.arange(0, max_f + 1, dtype=diep.float_th),
                requires_grad=True,
            )
        else:
            self.register_buffer("frequencies", torch.arange(0, max_f + 1, dtype=diep.float_th))
...
def spherical_bessel_smooth(r: Tensor, cutoff: float = 5.0, max_n: int = 10) -> Tensor:
    ...
    """
    n = torch.arange(max_n).type(dtype=diep.float_th)[None, :]
    r = r[:, None]
    fnr = (
        (-1) ** n
        * sqrt(2.0)
