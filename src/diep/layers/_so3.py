...
import diep
from diep.layers import MLP
from diep.utils.maths import binom, scatter_add
from diep.utils.so3 import generate_clebsch_gordan_rsh, sh_indices, sparsify_clebsch_gordon
...
class RealSphericalHarmonics(nn.Module):
    ...
    def _generate_Ylm_coefficients(self, lmax: int):
        ...
        m = torch.arange(1, lmax + 1, dtype=diep.float_th)[:, None]
        p = torch.arange(0, lmax + 1, dtype=diep.float_th)[None, :]
        mask = p <= m
        mCp = binom(m, p)
        ...
        l_id = torch.arange(0, lmax + 1, dtype=diep.float_th)[:, None, None]
        m = torch.arange(0, lmax + 1, dtype=diep.float_th)[None, :, None]
        k = torch.arange(0, lmax // 2 + 1, dtype=diep.float_th)[None, None, :]
...
class SO3TensorProduct(nn.Module):
    ...
    def __init__(self, lmax: int):
        ...
        cg = generate_clebsch_gordan_rsh(lmax).to(diep.float_th)
